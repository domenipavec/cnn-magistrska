...
#define BUFFER1 (MEM_BASE_ADDR + 0x00100000)
#define BUFFER2 (MEM_BASE_ADDR + 0x00300000)
...
int main()
{
	...
	int status = XAxiDma_BdRingCreate(txRing, TX_BD, TX_BD, XAXIDMA_BD_MINIMUM_ALIGNMENT, 10*2);
	if (status != XST_SUCCESS) {
	...
	status = XAxiDma_BdRingCreate(rxRing, RX_BD, RX_BD, XAXIDMA_BD_MINIMUM_ALIGNMENT, 10*layer_sizes[0]*layer_sizes[0]/4/RX_MAX_VSIZE);
	if (status != XST_SUCCESS) {
		std::cout << "Could not create bd ring" << std::endl;
		return 1;
	}
	...
	uint8_t *rxBuffer = (uint8_t *)BUFFER1;
	...
	for (int x = 0; x < n; x++) {
		...
		for (int p = 0; !(controls[x] & (1 << CTRL_STREAM_WEIGHTS)) && p < output_layers[x]; p += MAX_OUT_LAYERS) {
			if (output_layers[x]/MAX_OUT_LAYERS > 1) {
			...
			status = XAxiDma_BdRingAlloc(txRing, 2, &startTxBd);
			if (status != XST_SUCCESS) {
			...
			status = XAxiDma_BdSetBufAddr(txBd, (UINTPTR)weights[x]);
			if (status != XST_SUCCESS) {
			...
			XAxiDma_BdSetCtrl(txBd, XAXIDMA_BD_CTRL_TXEOF_MASK); // alst bd
			XAxiDma_BdSetARCache(txBd, 0x3); // cache default is 0b011
			...
			for (int rx_bd_index = 0; rx_bd_index < rx_bd_count; rx_bd_index++) {
				status = XAxiDma_BdSetBufAddr(rxBd, (UINTPTR)(rxBuffer + rx_bd_index*RX_MAX_VSIZE*3*output_layers[x]));
				if (status != XST_SUCCESS) {
					std::cout << "Could not set buffer address" << std::endl;
					return 1;
				}
				status = XAxiDma_BdSetLength(rxBd, out_layers*3, rxRing->MaxTransferLen);
				if (status != XST_SUCCESS) {
					std::cout << "Could not set hsize" << std::endl;
					return 1;
				}
				...
				XAxiDma_BdSetStride(rxBd, 3*output_layers[x]);
			...
			status = XAxiDma_BdRingToHw(txRing, 2, startTxBd);
			if (status != XST_SUCCESS) {
				std::cout << "Could not start transmit" << std::endl;
				return 1;
			}
