...
with tfnet.graph.as_default():
    ...
    with tf.Session() as sess:
        ...
        np.savetxt(input_buffer, scales)
        np.savetxt(input_buffer, adds)
        np.savetxt(input_buffer, weights.flatten())

        np.savetxt(input_buffer, img.flatten())

        print("Running full layer stack")
        cmd = subprocess.run(
            "../hls/cnn_full_layer_stack/solution1/csim/build/csim.exe",
